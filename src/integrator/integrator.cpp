...
SpectrumC Integrator::renderC(const Scene &scene, int sensor_id) const {
    ...
    if ( scene.m_opts.log_level ) {
        ...
        oss << "Rendered in " << duration_cast<duration<double>>
...
template <bool ad>
Spectrum<ad> Integrator::__render(
    const Scene &scene, 
    int sensor_id
) const {
    ...
    if ( likely(opts.spp > 0) ) {
        ...
        Vector2f<ad> samples_base = gather<Vector2f<ad>>(
            meshgrid(arange<Float<ad>>(opts.width),
                     arange<Float<ad>>(opts.height)),
            idx);

        Vector2f<ad> samples = 
            (samples_base + scene.m_samplers[0].next_2d<ad>()) /
            ScalarVector2f(opts.width, opts.height);

        Ray<ad> camera_ray = 
            scene.m_sensors[sensor_id] -> sample_primary_ray(samples);
...
void Integrator::render_primary_edges(
    const Scene &scene, 
    int sensor_id, 
    SpectrumD &result
) const {
    ...
    if (sensor->m_enable_edges) {
        PrimaryEdgeSample edge_samples = 
            sensor->sample_primary_edge(scene.m_samplers[1].next_1d<false>());
        ...
        // delta_L is the difference of different sides of the discontinuity
        SpectrumC delta_L = 
            Li(scene, scene.m_samplers[1], edge_samples.ray_n, valid) - 
            Li(scene, scene.m_samplers[1], edge_samples.ray_p, valid);

        // x_dot_n is the dot product between velocity and normal direction
        SpectrumD value = 
            edge_samples.x_dot_n * SpectrumD(delta_L / edge_samples.pdf);
